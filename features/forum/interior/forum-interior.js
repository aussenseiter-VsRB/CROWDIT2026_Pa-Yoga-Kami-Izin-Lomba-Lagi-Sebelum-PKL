...
import { getForumStatus, leaveForum } from '../../../js/services/forum-access.js';
import { getUsersForContext } from '../../../js/data/dummy-users.js';
import { isFollowing, toggleFollow, notifyNewMessage } from '../../../js/services/notifications.js';
import { API, LCG, LIMITS, DATA_PATHS, MOBILE_BREAKPOINT } from '../../../js/core/config.js';
...
function initForumUsers() {
  ...
  usersPromise = fetch(API.DUMMY_USERS_FORUM)
...
function AvatarStackHtml(memberCount, memberLimit, forumIndex) {
  ...
  const fallback = API.DUMMY_AVATAR_FALLBACK;
  let avatars = '';
  for (let i = 0; i < n; i++) {
...
export async function ForumInterior() {
  ...
  const memberLimit = forumType === 'group'
    ? (groupsRes.groups[groupIdx]?.maxMembers || LIMITS.DEFAULT_MEMBER_LIMIT)
