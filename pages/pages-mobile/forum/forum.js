...
export async function Forum() {
  ...
  const textChannels = forumData.channels.filter(c => c.type !== 'voice');
  const members = forumData.members || [];
  let activeChannel = textChannels[0] || forumData.channels[0];
  ...
  function renderChannelList() {
    ...
    el.querySelectorAll('.forum-channel-item').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const found = textChannels.find(c => c.id === id) || forumData.channels.find(c => c.id === id);
        if (found) {
          activeChannel = found;
          renderMessages();
        }
